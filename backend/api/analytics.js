...
analyticsRouter.get(
  "/rentaltotalsdt/:channelid/:fromdt/:thrudt",
  async (req, res, next) => {
    const { channelid, fromdt, thrudt } = req.params;
    try {
      const channelRentalTotalsDt = await getRentalsTotalsDates(
        channelid,
        fromdt,
        thrudt
      );
...
analyticsRouter.get(
  "/markettotalsdt/:vendorid/:fromdt/:thrudt",
  async (req, res, next) => {
    const { vendorid, fromdt, thrudt } = req.params;

    try {
      const vendorTotalsDt = await getMarketTotalsDates(
        vendorid,
        fromdt,
        thrudt
      );
...
analyticsRouter.get(
  "/videosfilter/:channelid/:fromdt/:thrudt",
  async (req, res, next) => {
    const { channelid, fromdt, thrudt } = req.params;
    try {
      const videosFiltered = await filterVideosByDate(
        channelid,
        fromdt,
        thrudt
      );
...
analyticsRouter.get(
  "/ordersfilter/:vendorid/:fromdt/:thrudt",
  async (req, res, next) => {
    const { vendorid, fromdt, thrudt } = req.params;
    try {
